...
## Vaihe 31.5 — Plugin-snapshotit ja palautus ⬜

**Tavoite**: Käyttäjä voi palauttaa rikkinäisen pluginin täydellisesti aiempaan tilaan. Snapshot on rajattu, nopea ja turvallinen, koska plugin on alun perin eristetty.

| # | Tehtävä | Tiedosto | Tila |
|---|---------|----------|------|
| 31.5.1 | Snapshot-rakenne: muisti, capabilityt, IPC, rekisterit | `kernel/snapshot.zig` | ⬜ |
| 31.5.2 | `sys_plugin_checkpoint(slot)` — tallenna tila | `kernel/syscall/snapshot_syscall.zig` | ⬜ |
| 31.5.3 | `sys_plugin_restore(slot, id)` — palauta tila | `kernel/syscall/restore_syscall.zig` | ⬜ |
| 31.5.4 | Inkrementaaliset snapshotit (vain muuttunut muisti) | `kernel/snapshot_delta.zig` | ⬜ |
| 31.5.5 | Watchdog: automaattinen palautus kaatumisen jälkeen | `kernel/plugin_watchdog.zig` | ⬜ |

**Testi**:
```bash
zig build run
# Odotettu serial:
# [Zinux] Plugin uptime checkpoint OK
# [Zinux] Simulating plugin crash...
# [Zinux] Crash detected
# [Zinux] Restoring from snapshot...
# [Zinux] Plugin uptime restored
# [Zinux] Other plugins unaffected
```
