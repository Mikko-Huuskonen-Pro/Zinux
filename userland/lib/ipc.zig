...
// Alin tason sys_ipc_flush — palauttaa poistettujen viestien määrän.
fn sysIpcFlush(slot: u32) i64 {
    // SYSCALL: RAX=num, RDI=slot.
    return asm volatile ("syscall"
        : [ret] "={rax}" (-> i64),
        : [num] "{rax}" (SYS_ipc_flush),
          [slot] "{rdi}" (@as(u64, slot)),
        : .{ .rcx = true, .r11 = true, .memory = true });
}
...
// Tyhjennä capability-slotin portin viestijono — palauttaa poistettujen viestien määrän.
pub fn flush(slot: u32) IpcError!u8 {
    // Kutsu kerneliä — vaatii recv-oikeuden slotissa.
    const ret = sysIpcFlush(slot);
    // Negatiivinen → virhe.
    if (core.isError(ret)) return mapSyscallError(ret);
